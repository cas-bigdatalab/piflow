...
def validate_docx(input_path: str) -> tuple[dict, str]:
    """
    验证DOCX文件格式有效性
    ...
    if not input_path.suffix.lower() == ".docx":
        result["warnings"].append("File extension is not .docx")
    ...
    try:
        with zipfile.ZipFile(input_path, "r") as zip_ref:
            ...
            if "word/numbering.xml" in namelist:
                result["warnings"].append("Contains numbering (check for proper list formatting)")
            ...
            for name in namelist:
                if name.endswith(".xml"):
                    try:
                        ...
                        result["warnings"].append(f"Non-UTF-8 content in {name}")
                    except Exception as e:
                        result["warnings"].append(f"Error reading {name}: {str(e)}")
    ...
    if result["valid"]:
        return result, "DOCX file is valid"
    else:
        return result, f"DOCX validation failed: {', '.join(result['errors'])}"


def main():
    parser = argparse.ArgumentParser(description="验证DOCX文件格式有效性")
    ...
    result, message = validate_docx(args.input_path)
