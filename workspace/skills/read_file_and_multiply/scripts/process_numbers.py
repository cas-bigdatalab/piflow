#!/usr/bin/env python3
"""
Multiply numbers by 10 processing script
Read numbers from text file, multiply each by 10, save to output file
"""
...
def read_file_and_add(filename, separator=None):
    """
    读取文本文件，按分隔符分割内容，并将每个数字乘以10
        ...
        for part in parts:
            ...
            if part:  # 确保不是空字符串
                try:
                    ...
                    numbers.append(num * 10)  # 每个数字乘以10
                except ValueError:
...
def process():
    ...
    # 处理文件
    result = read_file_and_add(input_file)
