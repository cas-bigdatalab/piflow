...
description: |
  数据集采样源节点。直接调用固定的数据集 HTTP 服务 `corpus.dataset.sampler` 接口，根据数据集 CSTR 随机采样指定数量的数据，并将接口响应保存为 JSON 文件。
...
output_params:
  - name: output
    type: json_file
    description: 数据集随机采样接口响应 JSON 文件
tag: 数据集采样
disciplinary_field: 数据集采样
publisher: COMMUNITY
---

## 功能概述

该 Skill 用于从指定数据集随机采样数据。脚本直接向 `http://172.31.3.81:7005/corpus.dataset.sampler` 发送 `GET` 请求，并通过查询参数传递 `cstr` 和 `size`。
...
`response` 保留上游接口的完整 JSON 响应，不改变采样记录字段。请求失败时输出 `success: false` 和 `error` 字段。
