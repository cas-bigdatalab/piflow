...
本文档描述 `piflow-skill-generator_planner` 在“skill 已生成后，用户又提供了新的成功流程”场景下的内部改写机制。该机制是生成器的内部参考能力，不作为独立对外 skill 发布。
...
- 出现了非平凡的技术、修复方案、变通方法（workaround）、调试路径或工具使用模式，并且未来会话会从中受益。记录并应用到重写skill中。
- 本次会话中加载或查阅过的某个技能被证明是错误的、缺少步骤的或已经过时的。立即修补它
...
内部改写机制当前提供的是**模板优先的最小重建**能力，而不是对旧 skill 文本做局部拼补：

1. 读取已有 skill 目录，提取可以复用的稳定事实，如技能名称、已有参数契约、脚本路径、已有 `skill.json` 字段。
2. 解析用户提供的新流程摘要或新的 rewrite spec。
3. 强制复用原 skill 名称。
4. 以模板为主骨架，把“已有稳定事实 + 新流程事实”合并成新的 rewrite spec，而不是把旧 skill 当成整体仿写底稿。
5. 以 `overwrite=True` 调用主生成器的生成与注册逻辑。
6. 输出改写后的 skill 目录和恢复出的 rewrite spec。

这意味着当前实现不是增量 patch 编辑器，而是：
...
- 即使改写输入来自旧脚本、旧流程或旧 skill，最终生成结果仍必须回到 `references/piflow_skill_template.md` 定义的结构，而不是沿用旧 skill 的整体写法。
- 若未来需要更细粒度的字段级 patch，再单独扩展；不要把当前最小实现伪装成增量编辑器，也不要把它变成对历史 skill 的整体仿写器。
