---
name: ingest_archive_zip_extractor
name_zh: ZIP 压缩包解压
description: 'ZIP 压缩包解压工具。递归扫描目录内 .zip 文件及分卷（.z01/.z02 等），合并分卷后解压并输出解压清单。当用户需要批量解压
  zip 或分卷 zip 时使用此 skill。不负责 7z、rar 等其他格式的解压。

  '
input_params:
- name: input_dir
  type: string
  required: true
  description: 待解压目录（递归）
- name: output_dir
  type: string
  required: true
  description: 解压输出根目录
- name: report
  type: string
  required: true
  description: 解压报告输出路径（JSON）
- name: max_entries
  type: string
  required: false
  default: "0"
  description: 单包最大解压文件数（0 不限）
output_params:
- name: output_dir
  type: directory
  description: 解压输出根目录
- name: report
  type: json_file
  description: 解压报告
tag: 采集
---

# ingest_archive_zip_extractor — ZIP 压缩包解压

## 功能概述

递归扫描目录内 `.zip` 文件，若存在同名前缀的 `.z01`/`.z02`/... 分卷文件，则合并后解压；否则按普通 zip 处理。输出解压清单与错误信息。

## 参数说明

| 参数 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `--input_dir` | 是 |  | 待解压目录（递归） |
| `--output_dir` | 是 |  | 解压输出根目录 |
| `--report` | 是 |  | 解压报告输出路径（JSON） |
| `--max_entries` | 否 | 0 | 单包最大解压文件数（0 不限） |

## 使用方法

```bash
python scripts/run_ingest_archive_zip_extractor.py \
  --input_dir <input_dir> \
  --output_dir <output_dir> \
  --report <report.json>
```

## 环境要求

使用 Python 标准库 `zipfile`，无需额外依赖。
