...
def upload_file(input_file, tool='SexDetermine'):
    """
    上传文件到服务器
    
    :param input_file: 输入文件路径
    :param tool: 工具名称，固定为SexDetermine
    :return: 文件ID
    """
    ...
    try:
        ...
        if result.get('code') != 0:
            ...
            print("可能的原因：")
            print("1. 文件格式不支持 - 服务器可能只接受 CSV 格式文件")
...
def start_analysis(file_id, species, tool='SexDetermine'):
    """
    启动性别判定分析
...
def format_result(result, output_dir):
    ...
    if content:
        ...
        # 展示前10个样本的判定结果
        print("前10个样本的判定结果：")
        for i, sample in enumerate(content[:10]):
            sample_id = sample.get('Sample ID', 'Unknown')
            gender = sample.get('Gender', 'Unknown')
            print(f"- 样本 {sample_id}: {gender}")
        print()
    
    # 下载结果文件
    if result_link:
        print("结果文件：")
        file_name = os.path.basename(result_link)
        full_link = f"https://www.bdbe.cn/kun/api/result/download?link={quote(result_link)}"
        print(f"- {file_name}")
...
def main():
    ...
    主函数
    """
    parser = argparse.ArgumentParser(description='单细胞样本性别判定')
    parser.add_argument('--input_file', type=str, required=True, help='输入单细胞样本数据文件路径')
