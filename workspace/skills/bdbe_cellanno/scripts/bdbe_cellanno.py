...
def upload_file(input_file, tool='CellAnno'):
    """
    上传文件到服务器
    
    :param input_file: 输入文件路径
    :param tool: 工具名称，固定为CellAnno
    :return: 文件ID
    """
    ...
    try:
        ...
        # 构建请求数据
        files = {
            'ds': (os.path.basename(input_file), open(input_file, 'rb'), 'application/octet-stream')
        }
        data = {
            'tool': tool
        }
        ...
        if result.get('code') != 0:
            ...
            print("可能的原因：")
            print("1. 文件格式不支持 - 服务器可能只接受 h5ad 格式文件")
...
def start_analysis(file_id, species, tool='CellAnno'):
    """
    启动细胞类型注释分析
...
def format_result(result, output_dir):
    ...
    if content:
        ...
        print("细胞类型分布：")
        for cell_type, count in cell_type_count.items():
...
def main():
    ...
    主函数
    """
    parser = argparse.ArgumentParser(description='单细胞测序数据细胞类型注释')
