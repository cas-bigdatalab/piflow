---
name: table_schema_structure_check
description: |
...
name_zh: table_schema_structure_check
input_params:
  - name: original_path
    type: string
    required: true
...
# table_schema_structure_check 格式一致性校验Skill
...
| 参数 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `--original_path` | 是 |  | 被检验表文件路径（需要校验的文件） |
| `--standard_path` | 是 |  | 标准化数据模式表路径（包含正确字段结构） |
| `--output_path` | 是 |  | 校验通过后输出文件路径 |
...
在 `scripts/` 目录下找到 `table_schema_structure_check.py`，使用Python执行：

```bash
python scripts/table_schema_structure_check.py --original_path <被检验表> --standard_path <标准模式表> --output_path <输出文件>
```
