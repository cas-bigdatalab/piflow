...
def test_materialize_direct_plan_uses_selected_replica_and_registered_skill_id():
    ...
    assert spec.result_node_id == DIRECT_SOURCE_NODE_ID
    ...
    assert plan.bind_result.center_of == {DIRECT_SOURCE_NODE_ID: "center-a"}
    assert len(plan.execution_dsl["nodes"]) == 1
    node = plan.execution_dsl["nodes"][0]
    assert node["skill"] == {
        "skill_id": "corpus_dataset_source_stop_1.0.0",
        "skill_name": "corpus_dataset_source_stop",
    }
    assert node["input_params"][0]["param_value"] == "replica-dataset-a"
    assert node["out_params"] == [
...
def test_submit_direct_plan_targets_selected_center_and_result_node():
    ...
    assert submission.result_node_id == DIRECT_SOURCE_NODE_ID
    ...
    # The actual engine converter accepts a source-only DAG with no bindings.
