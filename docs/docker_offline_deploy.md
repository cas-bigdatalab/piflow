...
```bash
docker load -i images/piflow-app.tar
...
```bash
docker images | grep -E 'piflow-app|postgres'
```
...
- `docker-compose.yml` 中已经声明了 `piflow-app` 和 `postgres:15`
...
因为 `piflow-app` 镜像依赖较多，现场构建时可能遇到：
