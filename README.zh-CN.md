...
当前应用镜像名称为 `registry.cn-hangzhou.aliyuncs.com/cnic-piflow/piflow:2.0.0`。
...
2. 前端无法连后端
   - 检查 `VITE_API_BASE`
   - 检查 `server.py` 是否监听在 `8080`

3. 附件图标能显示但下载失败
   - 检查附件路径是否真实存在于工作区
   - 检查 `/message/attach` 登记的路径是否正确

4. SSE 无输出
   - 检查浏览器网络面板中的 `/chat/stream`
   - 检查后端日志中 `chat stream request` 相关记录

## 当前文档边界

这份 README 以当前仓库实现为准，重点说明：
...
其中 `docs/workspace_file_api.md` 存在部分历史内容，使用时应以当前代码实现为准。
