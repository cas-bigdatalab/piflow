...
class PptxParser extends ConfigurableStop {
  ...
  override def perform(in: JobInputStream, out: JobOutputStream, pec: JobContext): Unit = {
    ...
    if (filePath.isEmpty) {
      println("########## Exception: can not parse, filePath is null!!!")
      throw new Exception("########## Exception: can not parse, filePath is null!!!")
    }
    if (fileSource.isEmpty) {
      println("########## Exception: can not parse, fileSource is null!!!")
      throw new Exception("########## Exception: can not parse, fileSource is null!!!")
    }
