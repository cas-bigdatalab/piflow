    ...
    val outDF = spark.sql(
      s"""
        ...
        |,split(parseAuthors(product_xml.product.pub_basic.authors),'\t<&\t')[0] as psn_name
         ...
         |,split(parseExtend(product_xml.product.pub_extend,product_xml.product.pub_basic.pub_type_id),'\t<&\t')[21] as issue_no1
         |,split(parseExtend(product_xml.product.pub_extend,product_xml.product.pub_basic.pub_type_id),'\t<&\t')[22] as issue_no2
         | from test
      """.stripMargin)
