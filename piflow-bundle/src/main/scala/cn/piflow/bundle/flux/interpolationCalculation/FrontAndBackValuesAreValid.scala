...
class FrontAndBackValuesAreValid extends ConfigurableStop{
  val authorEmail: String = "ygang@cnic.cn"
  val description: String = "The current value is invalid M(k), before and after the valid values M (k-1), M (k+1);Schema must contain hour, day_id fields (id incremented by date)"
  ...
  override def perform(in: JobInputStream, out: JobOutputStream, pec: JobContext): Unit = {
    ...
    val df= in.read()
    val resultDF = fluxInterpolation.ForwardAndBackwardInterpolation(spark,df,fields,-9999)
    out.write(resultDF)
