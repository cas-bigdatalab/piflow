...
class WildSpotCullingMDV_G extends ConfigurableStop{
  val authorEmail: String = "ygang@cnic.cn"
  val description: String = "Use sliding window mode, the standard deviation or variance is calculated " +
  ...
  override def perform(in: JobInputStream, out: JobOutputStream, pec: JobContext): Unit = {
    ...
    fields.split(",").foreach(x=>{
      df = flux.FluxOutfieldEliminationMDVSliding(spark,df,x,mdv_num.toInt,multiple.toDouble,statisticalType,flagValue)
    })
