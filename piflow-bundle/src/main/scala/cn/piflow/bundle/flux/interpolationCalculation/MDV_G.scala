...
class MDV_G extends ConfigurableStop{
  val authorEmail: String = "ygang@cnic.cn"
  val description: String = "Interpolation algorithm: MDV-G: sliding window;Schema must contain hour, day_id fields (id incremented by date)"
  ...
  override def perform(in: JobInputStream, out: JobOutputStream, pec: JobContext): Unit = {
    ...
    val mdvDF= in.read()
    val mdvGDF= flux.MDVSliding(spark, mdvDF,fields,mdv_num.toInt,qualityControlMarkerValue.toInt)
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val mdv_num = new PropertyDescriptor()
      .name("mdv_num")
      .displayName("mdv_num")
      .description("Window time,Must be odd")
      .defaultValue("7")
      .required(true)
      .example("7")
