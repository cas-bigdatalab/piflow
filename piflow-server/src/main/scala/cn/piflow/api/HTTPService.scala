...
object HTTPService extends DefaultJsonProtocol with Directives with SprayJsonSupport {
  ...
  def route(req: HttpRequest): Future[HttpResponse] = req match {
    ...
    case HttpRequest(GET, Uri.Path("/datacenter/flow/taskPlan"), headers, entity, protocol) => {
      ...
      val groupId = req.getUri().query().getOrElse("groupId", "")
      if (!groupId.equals("")) {
        val result = API.getDataCenterFlowTaskPlan(groupId)
        Future.successful(HttpResponse(SUCCESS_CODE, entity = result))
      } else {
        Future.successful(HttpResponse(FAIL_CODE, entity = "groupId is null or data center flow does not exist!"))
      }
    ...
    case HttpRequest(POST, Uri.Path("/hdfs/data"), headers, entity, protocol) => {
      try {
        ...
        case ex => {
          println(ex)
          Future.successful(HttpResponse(FAIL_CODE, entity = "get data from hdfs failed !"))
        }
    ...
    case HttpRequest(GET, Uri.Path("/flow/dataSize"), headers, entity, protocol) => {
      val appId = req.getUri().query().getOrElse("appId", "")
      if (appId != "" ) {
