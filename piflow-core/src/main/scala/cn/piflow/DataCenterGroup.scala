...
class DataCenterGroupExecutionImpl(group: Group, runnerContext: Context, runner: Runner) extends GroupExecution {
  ...
  val listener = new RunnerListener {
    ...
    def onDataCenterProcessFinished(flowName:String, appId: String) : Unit = {
      val localDataCenter = getLocalDataCenter()
      if(!dataCenterMap(flowName).contains(localDataCenter)){
        val time = new Date().toString
    ...
    mapGroupEntryWithConditions.foreach { en =>
      ...
      if (!startedProcessesAppID.contains(en._1) && en._2._2.matches(execution)) {
        ...
        if(flow.getDataCenter().equals("")){
          ...
          if(dataCenterMap(flowName).equals("")){
            val localDataCenter = getLocalDataCenter()
            dataCenterMap(flowName) = localDataCenter
            println("Set local dataCenter(" + localDataCenter + ") to flow " + flow.getFlowName() + "!!!!!!!!!!" )
  ...
  private def getLocalDataCenter() : String = {
    val localDataCenter = "http://" + ServerIpUtil.getServerIp() + ":" + PropertyUtil.getPropertyValue("server.port")
    localDataCenter
  }
}
